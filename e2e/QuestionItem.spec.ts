...
test.describe('QuestionItem Component Tests', () => {
    ...
    test('should delete question when Backspace is pressed on empty single-line question', async () => {
        // given
        const testQuestion: Question = { id: 'empty-question-id', text: '', answered: false, highlighted: false };
        const testQuestion2: Question = { id: 'empty-question-id2', text: '', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion, testQuestion2]);
        ...
        expect(updatedQuestions.find(q => q.id === testQuestion2.id)).toBeUndefined();
    });
    ...
    test('should NOT delete question when Backspace is pressed at first line of empty multiline question', async () => {
        // given
        const testQuestion: Question = { id: 'empty-multiline-question-id', text: '\n\n', answered: false, highlighted: false };
    ...
    test('should delete single empty line in multiline question when cursor is at the end', async () => {
        ...
        const updatedQuestion = (await appPage.getQuestions()).find(q => q.id === testQuestion.id);
        expect(updatedQuestion?.text).toBe('');
    });

    test('should delete character when Backspace is pressed in the middle of text', async () => {
        // given
        const testQuestion: Question = { id: 'test-question-id', text: 'Line1\nLine2', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion]);
        await appPage.reload();
    ...
    test('should navigate to next question on ArrowDown at end of text', async () => {
        // given
        const firstQuestion: Question = { id: 'first-question-id', text: 'First question\nWith two lines\n', answered: false, highlighted: false };
        const nextQuestion: Question = { id: 'next-question-id', text: 'Next question', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [firstQuestion, nextQuestion]);
        await appPage.reload();

        const questionItem = new QuestionItemPage(appPage.page, firstQuestion.id);
        const nextQuestionItem = new QuestionItemPage(appPage.page, nextQuestion.id);
    ...
    test('should navigate to previous question on ArrowUp at start of multiline question', async () => {
        // given
        const previousQuestion: Question = { id: 'previous-question-id', text: 'Previous question', answered: false, highlighted: false };
        const currentQuestion: Question = { id: 'current-question-id', text: '\n\nCurrent question', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [previousQuestion, currentQuestion]);
        await appPage.reload();

        const questionItem = new QuestionItemPage(appPage.page, currentQuestion.id);
        const previousQuestionItem = new QuestionItemPage(appPage.page, previousQuestion.id);
    ...
    test('should add new question below on Enter key when text is non-empty', async () => {
        // given
        const testQuestion: Question = { id: 'test-question-id', text: 'This is a question', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion]);
        await appPage.reload();
    ...
    test('should add new question when Tab is pressed at last question and text is non-empty', async () => {
        // given
        const testQuestion: Question = { id: 'last-question-id', text: 'Some text', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion]);
        await appPage.reload();
    ...
    test('should handle Tab key to move focus to next question', async () => {
        // given
        const firstQuestion: Question = { id: 'first-question-id', text: 'First question', answered: false, highlighted: false };
        const nextQuestion: Question = { id: 'next-question-id', text: 'Next question', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [firstQuestion, nextQuestion]);
        await appPage.reload();
    ...
    test('should handle Shift+Tab to move focus to previous question', async () => {
        // given
        const previousQuestion: Question = { id: 'previous-question-id', text: 'Previous question', answered: false, highlighted: false };
        const currentQuestion: Question = { id: 'current-question-id', text: 'Current question', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [previousQuestion, currentQuestion]);
        await appPage.reload();
    ...
    test('should update localStorage when text content changes', async () => {
        // given
        const testQuestion: Question = { id: 'test-question-id', text: 'Original text', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion]);
        await appPage.reload();
    ...
    test('should persist checkbox status in localStorage when toggled', async () => {
        // given
        const testQuestion: Question = { id: 'test-question-id', text: 'Question text', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion]);
        await appPage.reload();
    ...
    test('should maintain state after reloading the page', async () => {
        ...
        // then
        const reloadedQuestionItem = new QuestionItemPage(appPage.page, testQuestion.id);
    ...
    test('should add new question and persist it in localStorage', async () => {
        // given
        const testQuestion: Question = { id: 'test-question-id', text: 'This is a new question', answered: false, highlighted: false };
        await appPage.setLocalStorageData('questions', [testQuestion]);
        await appPage.reload();
